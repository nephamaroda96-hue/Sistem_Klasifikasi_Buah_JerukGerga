...
# Membaca file image_bf6d3d berbentuk PNG sesuai folder Anda
try:
...
# ==============================
# MODERN CSS UI ENHANCEMENT (Perbaikan Bug String CSS)
# ==============================
st.markdown(f"""
<style>
    /* Memaksa Penggunaan Font Times New Roman KECUALI untuk Ikon bawaan Streamlit */
    html, body, [data-testid="stAppViewContainer"], .stApp, p, label, li, h1, h2, h3, h4, h5, h6, input {{
        font-family: 'Times New Roman', Times, serif !important;
    }}
    
    /* Khusus untuk span dan button, pastikan font kustom TIDAK merusak class ikon (:not) */
    button, span:not([class*="Icon"]) {{
        font-family: 'Times New Roman', Times, serif !important;
    }}

    /* Memastikan font ikon bawaan Streamlit tetap menggunakan font aslinya */
    [data-testid="stIconMaterial"] {{
        font-family: 'Material Symbols Outlined' !important;
    }}
    
    /* Background Utama Menggunakan Gambar Pohon Jeruk Anda */
    .stApp {{
        {background_style}
    ...
    /* Memperbesar dan Mempertegas Tulisan Tombol Menu di Sidebar */
    ...
    /* Card Container Efek Kaca Transparan Organik */
    .custom-card {{
    ...
    /* Memperbesar Teks Format Berkas / Panduan Unggah */
    ...
    /* Ukuran Teks Utama di Seluruh Card */
    .custom-card h3 {{
    ...
    /* Modifikasi Tampilan Gambar agar Terlihat Premium */
...
# ==============================
# KONFIGURASI SIDEBAR NAVIGASI & BANNER PREMIUM (VERSI TERBARU)
# ==============================
with st.sidebar:
    # 1. Proses Membaca File Gambar Lokal 'sidebar_orange.png' untuk Logo Atas & Banner Bawah
    try:
        with open("sidebar_orange.png", "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read()).decode()
        img_src = f"data:image/png;base64,{encoded_string}"
    except Exception:
        img_src = ""  # Cadangan jika file gagal dimuat

    # 2. Logo dan Judul Aplikasi Utama (Ukuran Teks Diperbesar & Sinkron Gambar)
    if img_src:
        st.markdown(f"""
        <div style='display: flex; align-items: center; gap: 14px; margin-bottom: 30px; padding: 10px 5px;'>
            ...
            <div>
                <div style='font-weight: 800; font-size: 24px; color: #1E4620; line-height: 1.1; font-family: "Times New Roman", Times, serif;'>Sistem Pakar</div>
                <div style='font-weight: 700; font-size: 20px; color: #FF7A00; line-height: 1.1; font-family: "Times New Roman", Times, serif;'>Jeruk Gerga</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    else:
        # Cadangan emoji jika file gambar tidak ditemukan
        st.markdown("""
    ...
    # 3. Area Tombol Navigasi Menu (Termasuk Halaman Tentang)
    ...
    # Memberikan jarak vertikal fleksibel ke area bawah sidebar
    st.markdown("<br><br><br>", unsafe_allow_html=True)
    
    # 4. KARTU PROMOSI DI BAGIAN BAWAH SIDEBAR (Background Putih Bersih)
    if img_src:
        st.markdown(f"""
        <div style='background: #FFFFFF; 
                    ...
                    margin-bottom: 25px;'>
            <img src='{img_src}' style='width: 95px; height: auto; margin-bottom: 12px; filter: drop-shadow(0px 4px 6px rgba(0,0,0,0.08));'>
            <div style='color: #1E4620; font-weight: 700; font-size: 16px; margin-bottom: 6px; font-family: "Times New Roman", Times, serif;'>Jeruk Gerga Berkualitas</div>
            <div style='color: #555555; font-size: 13px; line-height: 1.4; font-family: "Times New Roman", Times, serif;'>
                Deteksi kualitas jeruk secara cepat, akurat, and mudah.
            </div>
        </div>
        """, unsafe_allow_html=True)
    
    # 5. Teks Hak Cipta / Copyright di Paling Bawah
    st.markdown("""
    <div style='text-align: center; color: #555555; font-size: 12px; font-family: "Times New Roman", Times, serif; padding-bottom: 10px;'>
        © 2026 Sistem Pakar Jeruk Gerga<br><span style='color:#888888;'>v2.0</span>
    </div>
    """, unsafe_allow_html=True)
...
database_hama = [
    ...
    {"nama":"Antraknosa", "gejala":"Kulit buah terdapat bercak hitam atau cokelat yang semakin meluas", "pengendalian":"Penyemprotan fungisida sesuai dosis anjuran dan menjaga kelembapan kebun"},
...
if st.session_state.page == "home":
    ...
    <div class='hero-banner'>
        <h1 style='margin:0; font-weight:800; font-size:38px;'>Sistem Informasi Kualitas Jeruk Gerga</h1>
        <p style='font-size:20px; opacity:0.9; margin-top:10px;'>
            Sistem berbasis Deep Learning dengan model Swin Transformer untuk mengklasifikasikan kualitas buah Jeruk Gerga.
        </p>
    </div>
    """, unsafe_allow_html=True)
    
    # --- CARD: FUNGSI UTAMA SISTEM ---
    st.markdown("""
...
# ==============================
# HALAMAN 2: INPUT CITRA (Layout Minimalis)
# ==============================
elif st.session_state.page == "input":
    st.markdown("<div class='main-title'>Unggah Citra Sampel</div>", unsafe_allow_html=True)
    st.markdown("<p style='text-align:center; font-size:20px; color:#555555; margin-top:-15px; margin-bottom:30px;'>Pilih metode untuk mengambil atau mengunggah citra jeruk jeruk gerga secara real-time</p>", unsafe_allow_html=True)
    
    # -------------------------------------------------------------
    # SEKSI 1: PILIH METODE UNGHAH (Dua Grid Card Utama)
    # -------------------------------------------------------------
    col_kamera, col_berkas = st.columns(2, gap="large")
    imgs = []

    with col_kamera:
        # Desain Card Kamera Langsung
        st.markdown("""
        <div style='background: rgba(255, 255, 255, 0.95); padding: 30px; border-radius: 16px; border: 1px solid rgba(46, 125, 50, 0.2); box-shadow: 0 6px 18px rgba(0,0,0,0.04); text-align: center; min-height: 290px;'>
            <div style='background: #E8F5E9; width: 70px; height: 70px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px;'>
                <span style='font-size: 35px;'>📷</span>
            </div>
            <h3 style='margin: 0 0 10px 0; font-size: 24px; font-weight: 700; color: #2E7D32;'>Kamera Langsung</h3>
            <p style='font-size: 16px; color: #666666; margin-bottom: 25px;'>Ambil gambar jeruk gerga secara langsung menggunakan kamera perangkat Anda.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.session_state.open_camera:
            ...
            cam = st.camera_input("Posisikan objek tepat di tengah bingkai kamera")
            if cam is not None:
                image = Image.open(cam).convert("RGB")
                imgs.append(image)

    with col_berkas:
        # Desain Card Unggah dari Perangkat
        st.markdown("""
        <div style='background: rgba(255, 255, 255, 0.95); padding: 30px; border-radius: 16px; border: 1px solid rgba(230, 81, 0, 0.2); box-shadow: 0 6px 18px rgba(0,0,0,0.04); text-align: center; min-height: 290px;'>
            <div style='background: #FFE0B2; width: 70px; height: 70px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px;'>
                <span style='font-size: 35px;'>📤</span>
            </div>
            <h3 style='margin: 0 0 10px 0; font-size: 24px; font-weight: 700; color: #E65100;'>Unggah dari Perangkat</h3>
            <p style='font-size: 16px; color: #666666; margin-bottom: 15px;'>Pilih dan unggah gambar jeruk gerga dari galeri atau folder lokal perangkat Anda.</p>
        </div>
        """, unsafe_allow_html=True)
        
        # File Uploader
        files = st.file_uploader(
            "Format berkas yang didukung: JPG, JPEG, atau PNG (Dapat memilih lebih dari satu berkas)",
            type=['jpg', 'png', 'jpeg'],
...
# ==============================
    ...
    st.markdown("<div class='main-title'>Proses Analisis Citra Sedang Berjalan...</div>", unsafe_allow_html=True)
...
# ==============================
    ...
    if not st.session_state.results:
        st.info("Informasi: Belum ada data hasil analisis. Silakan lakukan proses klasifikasi terlebih dahulu.")
    else:
        for i, r in enumerate(st.session_state.results):
            ...
            with c2:
                if r['grade'] == "Bukan Jeruk":
                    ...
                    st.error("Sistem mengidentifikasi bahwa objek pada citra bukan Jeruk Gerga, atau kondisi pencahayaan gambar terlalu rendah/kabur.")
                    ...
                    # Metric Section & Confidence Info
                    metric_col, logit_col = st.columns(2)
                    with metric_col:
                        st.metric(label="Tingkat Keyakinan (Confidence)", value=f"{r['score']} %")
                        st.markdown("""
                        <div class='info-box'>
                            Nilai confidence adalah nilai untuk mengukur tingkat keyakinan model terhadap gambar yang diunggah semakin besar confidencenya maka model semakin yakin bahwa jeruk itu masuk ke kelas tersebut.
                        </div>
                        """, unsafe_allow_html=True)
                    ...
                    with logit_col:
                        with st.expander("Nilai Output Model (Logits)", expanded=False):
                            ...
                            Nilai logits merupakan nilai mentah dari model sebelum diubah ke probabilitas.
                            </p>
                            """, unsafe_allow_html=True)
                            
                            st.code(f"""
Logit A (z1): {r['logits'][0]:.4f}
Logit B (z2): {r['logits'][1]:.4f}
Logit C (z3): {r['logits'][2]:.4f}
                            """)
                    
                    laporan += f"Sampel {i+1}: Grade {r['grade']} ({r['score']}%)\n"
                    laporan += f"Logits: A:{r['logits'][0]:.4f}, B:{r['logits'][1]:.4f}, C:{r['logits'][2]:.4f}\n\n"
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    with c1:
        st.download_button(
            "Unduh Laporan",
            data=laporan,
            file_name="Laporan_Klasifikasi_Jeruk.txt",
            mime="text/plain",
    ...
    # --- CARD 1: PENJELASAN APLIKASI ---
    st.markdown("""
    <div class='custom-card'>
        <h4>Apa itu Sistem Pakar Jeruk Gerga?</h4>
        <p>
            Aplikasi ini dibuat untuk membantu menentukan kualitas atau grade buah <b>Jeruk Gerga Lebong secara otomatis menggunakan bantuan kamera dan komputer. 
            Tujuannya adalah agar proses pemisahan jeruk bagus dan jeruk kurang bagus menjadi lebih adil, cepat, dan tidak tergantung pada perkiraan manual manusia saja.
        </p>
        <p><b>Kemampuan Utama Aplikasi:</b></p>
        <ul>
            <li><b>Bisa Periksa Banyak Gambar sekaligus:</b> Anda bisa memasukkan beberapa foto jeruk secara bersamaan agar proses pengecekan lebih hemat waktu.</li>
            <li><b>Tahu Grade secara Instan:</b> Sistem akan langsung memberi tahu apakah jeruk tersebut masuk ke dalam kategori Grade A, Grade B atau Grade C.</li>
            <li><b>Saran Penanganan Hama:</b> Jika hasil foto jeruk terdeteksi masuk kualitas rendah (Grade C), aplikasi akan memberikan tips atau cara merawat buah dari serangan hama/penyakit.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    
    # --- CARD 2: TEKNOLOGI YANG DIGUNAKAN ---
    st.markdown("""
    <div class='custom-card'>
        <h4>Teknologi yang Digunakan</h4>
        <p>Aplikasi ini dibangun menggunakan beberapa teknologi software berikut:</p>
        <ul>
            <li><b>Bahasa Pemrograman & Tampilan:</b> Menggunakan bahasa <i>Python</i> dan framework <i>Streamlit</i> untuk membuat halaman dashboard web yang mudah digunakan.</li>
            <li><b>Model Kecerdasan Buatan (AI):</b> Menggunakan library <i>PyTorch</i> dengan model Vision Transformer (Swin Transformer) yang bertugas mengenali bentuk dan tekstur kulit jeruk dari foto.</li>
            <li><b>Pengolah Gambar:</b> Menggunakan library <i>OpenCV</i> dan <i>Pillow</i> untuk mengatur ukuran foto, merapikan cahaya, dan menyiapkan gambar sebelum dibaca oleh model AI.</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)